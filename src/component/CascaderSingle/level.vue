...
watch(() => props.value, (newVal, oldVal) => {
  if (props.value?.length > 0) {
    selectItem.value = props.value?.map(item => {
      const temp = findNodeFromTreeById({ boDictId: '-1', dictName: '-1', children: props.options }, item, 'boDictId')
...
const getItem = (item) => {
  ...
  if (selectValue.value?.length > 0) {
    selectValue.value.forEach(temp => {
      if (temp?.endsWith(item.boDictId)) {
        isIn = true;
      }
    })
...
const handleCheck = (item) => {
  ...
  if (!item.checked && selectValue.value?.includes(item.boDictId)) {
    selectValue.value = [];
    selectText.value = [];
    selectItem.value = [];
    return
      ...
      <div class="content-wrapper">
        <div class="content-selects">
          <van-tag
            ...
            size="medium"
            type="primary"
            @close="handleDelete(selectItem)"
          >
            {{ selectItem?.map(temp => (temp.title || temp.dictName))?.join('/') }}
          </van-tag>
        </div>
