...
export const checkFireResistanceRating = (form, options, prompt = true) => {
  ...
  // 建筑结构
  if (filter?.[0]?.dictName === '钢砼结构' && ['三级', '四级', '低于四级'].includes(rate?.[0]?.dictName)) {
    form.fireBuilding.fireResistanceRating.warning = true
  }
  else if (filter?.[0]?.dictName === '砖混结构' && ['四级', '低于四级'].includes(rate?.[0]?.dictName)) {
    form.fireBuilding.fireResistanceRating.warning = true
  }
  ...
  if (form.fireBuilding.fireResistanceRating.warning === true) {
    ...
    prompt && showToast(`建筑结构为${filter?.[0]?.dictName}，耐火等级选择不合理，请修改或备注！`)
  }
  // 起火场所、建筑楼层
  const types = options.buildType?.filter(item => item.boDictId === buildType.value)
  if (firePlace?.text?.indexOf('居住场所') > -1 && buildFloor.value >= 19 && ['二级', '三级', '四级', '低于四级'].includes(rate?.[0]?.dictName)) {
    form.fireBuilding.fireResistanceRating.warning = true
    form.fireBuilding.fireResistanceRating.warningText = '居住场所，19<=总楼层数，耐火等级选择二、三、四级、低于四级不合理，请修改或备注！'
    // prompt && notification.open({ message: '填报异常提醒', description: '居住场所，19<=总楼层数，耐火等级选择二、三、四级、低于四级不合理，请修改或备注！', style: { backgroundColor: 'orange' } })
    prompt && showToast(form.fireBuilding.fireResistanceRating.warningText)
  }
  else if (firePlace?.text?.indexOf('居住场所') > -1 && (buildFloor.value < 19 && buildFloor.value >= 10) && ['三级', '四级', '低于四级'].includes(rate?.[0]?.dictName)) {
    form.fireBuilding.fireResistanceRating.warning = true
    form.fireBuilding.fireResistanceRating.warningText = '居住场所，10<=总楼层数<19，耐火等级选择三、四级、低于四级不合理，请修改或备注！'
    // prompt && notification.open({ message: '填报异常提醒', description: '居住场所，10<=总楼层数<19，耐火等级选择三、四级、低于四级不合理，请修改或备注！', style: { backgroundColor: 'orange' } })
    prompt && showToast(form.fireBuilding.fireResistanceRating.warningText)
  }
  else if (checkArray(firePlace?.text) && buildFloor.value >= 17 && ['二级', '三级', '四级', '低于四级'].includes(rate?.[0]?.dictName)) {
    form.fireBuilding.fireResistanceRating.warning = true
    form.fireBuilding.fireResistanceRating.warningText = '17<=总楼层数，耐火等级选择二、三、四级、低于四级不合理，请修改或备注！'
    // prompt && notification.open({ message: '填报异常提醒', description: '17<=总楼层数，耐火等级选择二、三、四级、低于四级不合理，请修改或备注！', style: { backgroundColor: 'orange' } })
    prompt && showToast(form.fireBuilding.fireResistanceRating.warningText)
  }
  else if (checkArray(firePlace?.text) && (buildFloor.value >= 9 && buildFloor.value < 17) && ['三级', '四级', '低于四级'].includes(rate?.[0]?.dictName)) {
    form.fireBuilding.fireResistanceRating.warning = true
    form.fireBuilding.fireResistanceRating.warningText = '9<=总楼层数<17，耐火等级选择二、三、四级、低于四级不合理，请修改或备注！'
    // prompt && notification.open({ message: '填报异常提醒', description: '9<=总楼层数<17，耐火等级选择二、三、四级、低于四级不合理，请修改或备注！', style: { backgroundColor: 'orange' } })
    prompt && showToast(form.fireBuilding.fireResistanceRating.warningText)
  }
  else if (firePlace?.text?.indexOf('厂房') > -1 && types?.[0]?.dictName === '高层' && ['三级', '四级', '低于四级'].includes(rate?.[0]?.dictName)) {
    form.fireBuilding.fireResistanceRating.warning = true
    form.fireBuilding.fireResistanceRating.warningText = '厂房火灾，建筑类别为高层时，选择三、四级、低于四级不合理，请修改或备注！'
    // prompt && notification.open({ message: '填报异常提醒', description: '厂房火灾，建筑类别为高层时，选择三、四级、低于四级不合理，请修改或备注！', style: { backgroundColor: 'orange' } })
    prompt && showToast(form.fireBuilding.fireResistanceRating.warningText)
  }
  else if (types?.[0]?.dictName === '地下' && ['二级', '三级', '四级', '低于四级'].includes(rate?.[0]?.dictName)) {
    form.fireBuilding.fireResistanceRating.warning = true
    form.fireBuilding.fireResistanceRating.warningText = '建筑类别为地下室，选择二、三、四级、低于四级不合理，请修改或备注！'
    // prompt && notification.open({ message: '填报异常提醒', description: '建筑类别为地下室，选择二、三、四级、低于四级不合理，请修改或备注！', style: { backgroundColor: 'orange' } })
    prompt && showToast(form.fireBuilding.fireResistanceRating.warningText)
  }
  if (form.fireBuilding.fireResistanceRating.warning === true) {
    form.fireBuilding.fieldWarning = form.fireBuilding.fieldWarning.replace('fireResistanceRating:false;', 'fireResistanceRating:true;')
...
export const checkAffectedHouse = (form, prompt = true) => {
  ...
  if (economicLoss.affectedHouse.value === 0 && basicInfo.severity.value === '2' && basicInfo.firePlace?.text?.indexOf('居住场所') > -1) {
    ...
    prompt && showToast('非轻微火灾，居住场所受灾户数为0不合理，请修改或备注！')
  }
  else {
...
export const checkFireInjuryCost = (form, prompt = true) => {
  ...
  if (economicLoss.fireInjuryCost.value > 0
    ...
    prompt && showToast('无人员伤亡，人身伤亡支出费用应该为0，请修改或备注！')
  }
  else {
...
export const checkFireDisposalCost = (form, prompt = true) => {
  ...
  if (economicLoss.fireDisposalCost.value === 0 && basicInfo.severity.value === '2') {
    ...
    prompt && showToast('非轻微火灾，现场处置费用为0，不合理，请修改或备注！')
  }
  else {
...
export const checkBurnedArea = (form, prompt = true) => {
  ...
  && severity.value === '1' && fireType?.text?.includes('建构筑物火灾')) {
    ...
    prompt && showToast(form.basicInfo.burnedArea.warningText)
  }
